...
describe('Bus', function (emitter) {
    ...
    describe('.off()', function (sub) {
        ...
        describe('.off([event,] fn)', function () {
            it('should remove the subscription from the event', function () {
                hasListeners(['event','nested'], noop)
                emitter.off('event.nested', noop)
                notListeners(['event','nested'], noop)
            })
            it.skip('should return all matching subscriptions', function () {
        ...
        describe('.off(event)', function () {
            ...
            it('should remove all disjuncted topics', function () {
                ...
                emitter.a._subs.should.not.include(sub)
                emitter.b._subs.should.not.include(sub)
                emitter.c._subs.should.include(sub)
            })
        })
        describe('.off([event,] subscription)', function () {
            ...
            it('should not effect topics that don\'t contain the subscription', function () {
                emitter.off(sub)
                emitter.event.nested._subs.should.include(sub)
            })
            it('should remove from the specified topic', function () {
                emitter.off('event.nested', sub)
                emitter.event.nested._subs.should.not.include(sub)
            })
        })
        ...
        describe('.off([event,] string)', function () {
            it.skip('should remove all subscription will named functions matching the string', function () {
                
            })
            it.skip('Can unsubscribe anonamous functions while leaving named functions', function () {
                expect(2)
    ...
    describe('Subscription', function () {
        describe('.once()', function () {
            it('should remove the subscription as after one call', function () {
